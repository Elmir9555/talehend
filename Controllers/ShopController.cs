...
namespace OganiProject.Controllers
{
    public class ShopController : Controller
    {
        ...
        public async Task<IActionResult> GetProductsWithPrice(int minValue, int maxValue)
        {

            var list = await _context.Products.Where(x=> x.Status!= DataStatus.Deleted).Where(m => m.Price >= (decimal)minValue && m.Price <= (decimal)maxValue).ToListAsync();

            return View("ShopPage", list);
        }
